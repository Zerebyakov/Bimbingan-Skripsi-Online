...
const ListPengajuan = () => {
  ...
  const handleOpenModal = (item, isUpdate = false) => {
    ...
    if (isUpdate) {
      setForm({
        ...
        dosenId3: item.dosenId3 || item.Pembimbing3?.id_dosen || "",
      });
    } else {
      setForm({ dosenId1: "", dosenId2: "", dosenId3: "" });
    }
  };
    ...
    try {
      const res = await axios.put(
        `${baseUrl}admin/pengajuan/${selectedPengajuan.id_pengajuan}/assign-dosen`,
        {
          ...
          dosenId3: form.dosenId3 ? Number(form.dosenId3) : null,
        },
        {
          withCredentials: true,
  ...
  // Unduh file proposal
  const handleDownloadProposal = (proposalUrl, title) => {
    if (!proposalUrl) {
      alert("File proposal belum diunggah oleh mahasiswa.");
      return;
    }
    const fileName = title.replace(/\s+/g, "_");
    const downloadUrl = `${baseUrl}uploads/${proposalUrl}`;
              ...
              <form onSubmit={handleAssignDosen} className="p-6 space-y-5">
                ...
                <div className="flex justify-end gap-2 border-t border-gray-200 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={
                      !form.dosenId1 && !form.dosenId2 && !form.dosenId3
                    }
